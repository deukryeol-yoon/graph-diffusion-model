...
class GraphEmbeddings(Dataset):
    ...
    def __init__(self, name, root_dir, model_path, image_size=16, features="id", transform=None, device=None):
...
def get_dataset(args, config):
    ...
    if config.data.dataset in ["QM7b", "MUTAG", "QM9", "ZINC"]:
        ...
        dataset = GraphEmbeddings(name=config.data.dataset, root_dir=config.data.root_dir, model_path=config.data.model_path, image_size=config.data.image_size, device=config.device)
