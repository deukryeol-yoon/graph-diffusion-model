...
real_graphs = []
for graphset in [TUDataset(name = 'MUTAG', root = './dataset')]:
...
def train(model, dataloader, args, epochs=5000):
    ...
    with open('graph_embeddings_MUTAG.pkl', 'wb') as f:
      pickle.dump(graph_embeddings, f)
    files.download('graph_embeddings_MUTAG.pkl')
...
def main():
    ...
    train(model, dataloader, args, epochs=15)

    with open('model_MUTAG.pkl', 'wb') as f:
      pickle.dump(model, f)
    files.download('model_MUTAG.pkl')
